...
**Deploy-fix (1 jul, na sessie 4):** de sessie-4 code (instant optimistic card + premium `MyStoneCard`) was wél gepushed maar nog **niet gedeployed** — laatste prod-deploy was nog sessie-3 (`e30ddbd`, 12:24). Deploy is hier **handmatig** (`vercel --prod`, geen GitHub auto-deploy), dus die stap ontbrak. Nu gedraaid → deploy `vibekilled-487mg4gao`, aliased naar vibekilled.rip. **Alles staat nu écht live.** Reminder: na committen altijd `vercel --prod`.
Nieuw (1 jul, sessie 4): **eigen card = de dopamine hit, verschijnt OP DE FRAME dat je op "Drop my pin" klikt** — geen netwerk meer ertussen. **Optimistic drop**: `dropPin` bouwt lokaal een volledige pin (`buildOptimisticPin`), seedt de cache, opent de card en **snapt** de map er direct heen (nieuwe `snap`-modus in `MapView` = `setView`, geen 1s-glide); de server bevestigt op de achtergrond → `reconcileDrop` wisselt naadloos het echte id/coords/vlag om, of `rollbackDrop` haalt 'm terug met een toast (rate-limit/cooldown). Coarse locatie wordt nu voor iedereen bij load opgehaald zodat de optimistic pin landt waar de server 'm plaatst (**geen sprong** bij reconcile, ook bij resurrected re-drop). `KillModal` is nu **pure form** (geen await/spinner) → emit `DropForm`. Card zelf **premium** (`PinPopup` → `MyStoneCard` vs `VisitorCard`): provider-glow-halo, pulserende **★ You**-chip, grote gloeiende naam, **klok-stijl hero-countdown**, grand-entrance-animatie (`vk-mine-reveal`). `DropFlash` afgeslankt tot een snelle boom die van bovenaf omhoog wegvliegt — de card is de ster, nooit achter een full-screen skull.
Eerder (1 jul, sessie 3): **eigen card = je universum achter de muur, opent INSTANT**. De arcade (Campfire of Hope-presence + **Tetris** + **LOLReads**) leeft nu **enkel in je eigen map-popup**, die **zichzelf opent op mount** (de marker `openPopup()`t van binnenuit → geen ref/timing-race meer) zodat 'ie meteen na een drop én bij elke reload verschijnt. Verder: **browser-notificaties bij inkomende sympathy/Good4U/🤝** (`fireReaction`, achtergrond-tab, `useMyPin` pollt door in background), **"X also behind the wall"-badge** (persoon-SVG + pulse) op elke card, admin toont **geven-land → ontvangst-land** (`🇳🇱 → 🇧🇷`) + **Tetris high-score per dev op de Vibe Kings-leaderboard**. Opgeruimd: **StartupBar-widget eruit** en **alle dode chat-code verwijderd** (routes/store/api/hooks/types/admin) — presence (`joinCampfire`/"around the fire") blijft.
...
## ACTIVE WORK / NEXT
- [x] **Instant optimistic drop** — card verschijnt op de klik-frame (geen netwerk-wait): `dropPin`/`buildOptimisticPin`/`openMyCard` + server-reconcile/rollback op de achtergrond; `MapView` `snap`-focus; coarse loc voor iedereen bij load (geen reconcile-sprong)
- [x] **Premium eigen card** — `PinPopup` gesplitst in `MyStoneCard` (glow-halo, ★ You-chip, gloeiende naam, klok-hero-countdown, `vk-mine-reveal`) vs compacte `VisitorCard`; `KillModal` = pure form (`DropForm`); `DropFlash` afgeslankt tot snelle boom
- [ ] Live testen: voelt de drop→card écht instant op device? DropFlash-boom niet te veel/te weinig? Card-vibe genoeg "wow"?
- [ ] Edge: netwerk >3s → tijdelijke pin 404't bij `useMyPin`-refetch vóór reconcile (self-healt bij reconcile) — checken of dit ooit zichtbaar knippert
